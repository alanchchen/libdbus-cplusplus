...
#else
#if 0
struct PathTraits : public std::char_traits<char> {
...
typedef std::basic_string<char, PathTraits> Path;

struct SignatureTraits : public std::char_traits<char> {
  static bool lt(char one, char two) {
    return one < two;
  }
...
typedef std::basic_string<char, SignatureTraits> Signature;
#else
