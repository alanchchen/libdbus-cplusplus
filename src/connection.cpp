...
Connection::Connection(const char *address, Dispatcher *dispatcher, bool priv)
  : _timeout(-1)
{
  InternalError e;
...
unsigned long Connection::unix_user() const
{
  ...
  dbus_bool_t b = dbus_connection_get_unix_user(_pvt->conn, &uid);
...
{
  return dbus_connection_get_is_authenticated(_pvt->conn);
}
