...
/*! Generate adapter code for a XML introspection
  */
void generate_adapter(Xml::Document &doc, const char *filename)
{
  ostringstream body;
  ostringstream head;
  ...
  for (Xml::Nodes::iterator i = interfaces.begin(); i != interfaces.end(); ++i)
  {
    ...
    // a "Adapter" is added to class name to distinguish between proxy and adapter
    ifaceclass += "Adapter";
