...
{
  cerr << endl << "Usage: " << basename(argv0) << " <xmlfile> [ --proxy=<outfile.h> ] [ --adapter=<outfile.h> ]"
       << endl << endl;
  exit(-1);
}
...
int main(int argc, char **argv)
{
  ...
  for (int a = 1; a < argc; ++a)
  {
    ...
    else if (!strncmp(argv[a], "--adapter=", 10))
    {
      adapter_mode = true;
      adapter = argv[a] + 10;
    }
  ...
  if (!proxy_mode && !adapter_mode) usage(argv[0]);
  ...
  if (adapter_mode) generate_adapter(doc, adapter);
